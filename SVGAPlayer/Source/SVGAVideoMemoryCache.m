...
- (void)cutomeInit {
    ...
    _videoWarningLimit = 20 * 1024 * 1024;
...
- (void)setShowMemoryCostTip:(BOOL)showMemoryCostTip {
    _showMemoryCostTip = showMemoryCostTip;
    // 走一次setter方法显示或隐藏tip
    self.totalMemoryCost = self.totalMemoryCost;
}
...
#if defined(DEBUG) || defined(INHOUSE)
        if (_videoWarningLimit > 0 && cost >= _videoWarningLimit) {
            [SVGADebugAlert showAlertWithEntity:object cost:cost];
        }
#endif
    }
}
...
- (void)setTotalMemoryCost:(NSUInteger)totalMemoryCost {
    ...
    if (_showMemoryCostTip) {
        ...
        void(^removeView)() = ^{
            if (_memoryCostTip && self.memoryCostTip.superview) {
...
- (CGRect)memoryCostTipFrame {
    if (CGRectEqualToRect(_memoryCostTipFrame, CGRectZero)) {
        _memoryCostTipFrame = CGRectMake(10, [UIScreen mainScreen].bounds.size.height - 75, 150, 15);
    }
...
- (void)setCountLimit:(NSUInteger)countLimit {
    if (_countLimit > 0) {
        ...
        self.strongCache.countLimit = countLimit;
    }
}
