...
- (void)stepToFrame:(NSInteger)frame {
    ...
    if (frame < self.frames.count) {
        ...
        if (frameItem.alpha > 0.0) {
            ...
            self.frame = SVGARectToCGRect(frameItem.layout);
            self.transform = CATransform3DMakeAffineTransform(SVGAAffineTransformToCGTransform(frameItem.transform));
...
- (void)resetTextLayerProperties:(NSAttributedString *)attributedString {
    ...
    UIColor *textColor = textAttrs[NSForegroundColorAttributeName];
    if (textColor != nil) {
        self.textLayer.foregroundColor = textColor.CGColor;
    }
    
    UIFont *font = textAttrs[NSFontAttributeName];
    if ([font isKindOfClass:[UIFont class]]) {
        self.textLayer.font = (__bridge CFTypeRef)(font.fontName);
